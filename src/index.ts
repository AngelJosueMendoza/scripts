...
const validateQrs = async () => {
  try{
    ...
    if(!orders.statusCode || orders.statusCode >= 300) return console.error("No se encontraron ordenes")
    for(const order of orders.body.results){
      const services: ShipmentData = order.custom?.fields["services"] && JSON.parse(order.custom?.fields["services"])
    ...
    console.log("Qrs actualizados")
    console.log("Actualizando orden")
    await apiRootQA.orders().withId({ID: order.id}).post({
      body: {
